...
// func InitNotifications(deps *dependencies.Container) {
// 	notes, err := deps.Services.Note.GetAllWarn()
// 	if err != nil {
// 		return
// 	}
...
// 		if note.Url != nil && *note.Url != "" {
// 			builder.WriteString(fmt.Sprintf(
// 				`<br><a href="#" onclick="window.openUrl('%s')">%s</a>`,
// 				*note.Url, *note.Url))
// 		}
// 		builder.WriteString("</div>")
// 	}

// 	builder.WriteString(`</div>`) // Cierre de .grid
// 	builder.WriteString(`<div class="actions"><button onclick="window.close()">Aceptar</button></div>`)
// 	// builder.WriteString(`<div class="actions"><button onclick="window.external.invoke('close')">Aceptar</button></div>`)
// 	builder.WriteString("</body></html>")
...
// 			window.external.invoke("open:" + url);
// 		}
...
// 	w.Bind("close", func() {
// 		fmt.Println("cerrar") // <- debería verse en consola
// 		w.Dispatch(func() {
// 			w.Terminate()
// 		})
// 	})
